...
@dp.message_handler(lambda message: message.text in ["Выбрать период", "Ввести конкретную дату", "Последний день", "Последние 3 дня", "Назад", "К вводу каналов"])
async def handle_period_choice(message: types.Message):
    ...
    elif message.text in ["Последний день", "Последние 3 дня"]:
        if message.text == "Последние 3 дня":
            period_days = 3
        elif message.text == "Последний день":
            period_days = 1
